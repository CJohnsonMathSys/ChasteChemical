...
void printVecVec(std::vector<std::vector<bool>> VecVec){
  for (unsigned int i = 0; i < VecVec.size(); i++)
  {
    for (unsigned int j = 0; j < VecVec[i].size(); j++)
    {
...
double deltaGtoKf(double deltaG, double Temp, double Kr = 1.0){

  double Rkj = 8.3144598e-3;

  return Kr*exp(-deltaG/(Rkj*Temp));
}

double KftodeltaG(double Kf, double Kr, double Temp){

  double Rkj = 8.3144598e-3;
  return -Rkj*Temp*log(Kf/Kr);
}

double calculateGibbsFromQuotient(double G0, double Q, double Temp){

  return G0 - 8.3144598e-3*Temp*log(Q);
};
